...
money_list_xls=ExcelReader(excel,'money_list').data
user_name=Config().get('account')
My_http=My_http()
My_db=My_db()
member_id=My_db.get_member_id(user_name)
@paramunittest.parametrized(*money_list_xls)
class Money_list(TestCase):
    '''
    收益明细接口
    '''
    ...
    def test_money_list(self):
        # 设置url
        wallet_list = Config(url_file).get('money_list')
