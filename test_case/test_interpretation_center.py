...
interpretation_center_xls=ExcelReader(excel,'interpretation_center').data
My_http=My_http()
@paramunittest.parametrized(*interpretation_center_xls)
class Interpretation_center(TestCase):
    '''
    解读大厅接口测试
    '''
    def setParameters(self, case_name, method, token,result, code,amountMode,improvStatus,notWinRefund,ownBuyAmount,publishType,raceStatus,raceType):
        '''
        设置params
        :param case_name:
        :param method:
        ...
        self.improvStatus = improvStatus.split(',')
    ...
    def test_wallet_list(self):
        # 设置url
        interpretation_center = Config(url_file).get('interpretation_center')
        self.url=interpretation_center
        ...
        data={'amountMode': self.amountMode,
              'improvStatus': self.improvStatus,
